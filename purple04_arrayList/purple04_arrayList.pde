...
void setup() {
  ...
  for (int i=0; i<particle.length; i++) {
    particle[i] = new Particle(new PVector(random(0, width), random(0, height)), 2, 10, 5);
  }
...
void draw() {
  ...
  pizza.text(newLetter, pizza.width/2, pizza.height/2-50); //slight offset from center on the Y to make it feel more centered vertically
  pizza.endDraw();
